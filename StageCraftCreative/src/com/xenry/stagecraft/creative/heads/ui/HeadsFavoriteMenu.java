 ...
 * is prohibited.
 */
public class HeadsFavoriteMenu extends Menu<Creative,HeadsManager> {
	
	public HeadsFavoriteMenu(HeadsManager manager, String uuid){
		super(manager, "head-favorite-" + uuid, "Favorite Heads", 6);
	}
	
	@Override
	protected void init(Player player, MenuContents contents) {
		List<Head> heads = manager.getFavoriteHeads(player);
		if(heads.size() < 1){
