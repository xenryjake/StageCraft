package com.xenry.stagecraft.skyblock.gameplay.pvptoggle;
 ...
 * is prohibited.
 */
public final class PvPCommand extends Command<SkyBlock,GameplayManager> {
	...
	protected void serverPerform(CommandSender sender, String[] args, String label) {
		...
		if(args[0].startsWith("on") || args[0].startsWith("en")){
			...
			value = profile.getSetting(Setting.SKYBLOCK_PVP_ENABLED);
			sender.sendMessage(M.elm + target.getName() + M.msg + "'s PvP is currently " + M.onOff(value) + M.msg + ".");
			return;
		}
		profile.setSetting(Setting.SKYBLOCK_PVP_ENABLED, value);
	...
	protected void playerPerform(Profile profile, String[] args, String label) {
		...
		if(args[0].startsWith("on") || args[0].startsWith("en")){
			...
			value = targetProfile.getSetting(Setting.SKYBLOCK_PVP_ENABLED);
			profile.sendMessage((profile == targetProfile ? M.msg + "Your" : M.elm + target.getName() + M.msg + "'s")
					+ " PvP is currently " + M.onOff(value) + M.msg + ".");
		}
		targetProfile.setSetting(Setting.SKYBLOCK_PVP_ENABLED, value);
